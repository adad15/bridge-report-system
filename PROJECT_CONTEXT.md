...
- 模块 3 最初建立了 `BridgeAnnualInspectionData` 跨端契约；当前运行时已统一升级为 2.0，仅保留导入和校对候选，删除 Word 扣分、导入评分及人工二选一状态，Python/C++/TypeScript 均严格拒绝 1.x。
...
- 合同 1.2 和模块 6 早期评分方案仅保留在历史规格/迁移记录中；运行时代码、当前样例和数据库最终态均以 2.0 与系统评分为准。

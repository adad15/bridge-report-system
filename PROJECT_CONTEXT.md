...
报告复用这项“只写增加、减少或持平”的展示能力，但不能直接复用其 `count(defect_observations)` 口径：范围拆分会把一条来源病害展开成多条观测。报告第一版使用 `source_defect_count_delta_v1`，按来源病害去重，并允许用户手动选择 `inspection_years.report_comparison_inspection_id`（迁移 `030` 从 `previous_inspection_id` 改名而来）；相应通用查询仍需新增。
...
- 第三章业务数据
- 第五章业务数据
- 附录二桥梁基本状况卡片的档案数据（版式已按正式卡片生成，九十六格里没有数据来源的一律留空）
- 维修养护记录及维修效果确认
- 电子签名
- Milvus、AI 润色和事实校验
...
10. 第一版标准模板的第三章保留标题及版式，正文为空；两张附录卡片版式齐全，没有数据来源的格子留空。
11. 生成结果只是临时下载文件；系统不保存报告版本或永久归档 Word。任务表 `report_generation_jobs` 是运行设施：默认保留 24 小时可下载，到期删文件转 `expired` 并清空路径、文件名与诊断正文，终态行再留 7 天后物理删除。
...
- 报告生成 §27 十步全部完成：迁移 032（模板/人员/设备/年度配置/生成任务）、033（交付文件名）；接口在 `/api/report/*`、`/api/inspection-years/{id}/report-*`、`/api/report-jobs/*`；页面入口为侧边栏「报告模板」「报告人员与设备」和年度工作台的「生成报告」。
- 报告 §1.1 桥梁概况（§27 第 11 步）：迁移 034 给 `bridges` 加了十七列档案字段（斜交角、行车道宽、人行道宽、桥面铺装、伸缩缝形式与设缝墩号、支座、上部结构、每孔片数、梁高、桥台、桥墩、基础、设计荷载、设计/施工/监管单位）；`GET/PUT /api/bridges/{id}/profile` 读写，写是整体覆盖；报告 §1.1 改出叙述段落（不再是两列表，那张表附录2 卡片里已经有了），同一批字段顺带填上附录2 卡片的十三格；录入入口是桥梁总览页的「桥梁概况」卡片（取代原来的「最近动态」）。图片（地理位置图、桥型布置图、桥梁照片）不在这一步。
- 真实规模验收实测（百股大桥 2024/2026）：59 页 166 图 / 101 页 455 图，生成总时长 68.6 s / 110.5 s，Python 峰值 123 MB / 182 MB，文件 5.9 MB / 21.3 MB，Office 打开 0.8 s / 1.0 s。§26 十三条通过十二条。
- **唯一待补项**：本机 `Word.Application` 与 `KWPS.Application` 都指向 WPS，没有装真 Microsoft Word，§26 第 4 条的 Word 侧需在装有真 Word 的机器上补测。另外库里只有百股大桥一座桥，34 页那一档的常规规模数据尚未导入。

...
> 2026-09-04 现行基线：模块 04 已实施，当前输出为 `BridgeAnnualInspectionData 5.0`。Word 导入
> 只抽取第二章病害表和病害照片，不读取第四章评分，不复算构件分，也不会因为缺少评分表而失败。
> 构件解析和评分树解析由 C++ 关系化解析工作区完成。下文关于合同 1.2、`ratings`、Word 扣分、
> 第四章评分解析和评分差异校对的内容均为历史实现记录，不适用于当前运行时。
...
1. 输入只支持 `.docx`，由 C++ 先归档或建立受控临时源，再把受控路径交给 Python。
2. 当前 Word 规则配置为 `辽宁国省干线`，由调用方显式传入，Python 不自动猜模板。
3. 解析可信区域为第二章三类病害检查表和按照片编号识别的病害图片区。
4. 输出病害来源事实、尺寸原文、照片候选、warnings/errors 和临时图片清单。
5. 输出必须通过合同 5.0 Pydantic 模型；不含 Word 评分及数据库构件/评分树绑定字段。
6. Python 不写 PostgreSQL，不决定年度修订，不生成历史对比或正式报告。
7. C++ 负责归档照片、保存候选 JSON、关系化解析、人工校对和正式确认。
8. 真实辽宁样例的当前基线仍关注病害和照片数量，不再包含“15 个评分项”。
...
| 2026-07-14 | 合同 1.2 解析实施完成：`defect_tables.py` 新增 标度/病害扣分/构件评分 列定位与按构件组传播（兼容合并单元格复制到每行与仅组首行有值两种形态，每组只产出一条评分候选）；`component_ratings.py` 依据 4.1.1 纯函数复算并自动分类校验状态；真实样例保持 25/31/36 基线并新增 65/65/55.81 精确断言 | 实施合同 1.2 | 模块 04、05、06 |
| 2026-08-10 | 删除 Word 评分解析 | 评分改为系统根据正式病害、构件档案和版本化规范自主计算 | 模块 03～06 |
| 2026-08-28 | 输出升级为合同 5.0 | 构件和评分树解析状态移入关系表 | 模块 03～06 |
| 2026-09-04 | 补充当前实现基线 | 避免历史评分章节继续指导运行时开发 | Word 导入 |

...
> 2026-09-04 现行基线：校对工作台运行时合同为 5.0。页面编辑来源病害事实和照片关系；构件解析、
> 多目标/区间实例以及评分树解析由独立关系化工作区和命令维护，普通整份草稿保存不能覆盖这些
> 权威状态。Word 评分校对、来源分/复算分二选一和照片逐张确认已经删除。已确认记录的评定页读取
> 入库时保存的正式 `assessment_run` 结果，不重新读取 Word 评分。下文冲突内容仅作历史记录。
...
1. `parsed_result_json` 只保存合同 5.0 的来源事实和一般校对事实。
2. 构件组、解析目标、病害解析实例和评分树解析保存在迁移 027～029 建立或收紧的关系结构中。
3. 手工新增病害使用专用命令，在一个事务内同时创建来源事实与人工解析结果。
4. 普通草稿保存使用 `draft_version`，并显式拒绝已移出合同的解析字段。
5. 预检和确认由 C++ 合并来源事实与关系表状态；正式病害必须绑定已确认构件档案中的实际构件和有效评分树节点。
6. 照片关系随病害组确认；照片没有独立确认状态。
7. 确认入库写正式病害、尺寸和照片，然后保存系统自主评定结果；不写 Word 评分。
8. 已确认记录进入只读态，重新打开时读取已保存的正式评定报告。
...
允许改评分值和等级。部件、结构分部和全桥总分不自动重算；构件评分必须根据已确认的病害扣分自动复算校验。

### 7.4 来源证据弹窗

主校对区不常驻右侧证据栏。用户选中候选后点击“查看来源证据”，通过模态弹窗显示：
...
证据字段默认只读；点击关闭按钮或弹窗外部关闭。该设计为病害和照片主校对区释放水平空间。
...
部件、结构分部和全桥评分第一版不重新计算。构件评分根据 JTG/T H21-2011 第 4.1.1 条复算，并显示 Word 来源值、复算值、最终确认值和校验状态。
...
部件、结构分部和全桥层级只保存 Word 或人工校对后的评分和等级。构件层级保存 Word 来源值、规范复算值、最终确认值、校验状态和计算明细；`score` 为最终确认值。

构件评分不一致时，用户必须显式选择“接受 Word 值”或“采用复算值”并填写原因，否则入库前检查失败。无法复算时必须补齐/修正扣分，或显式确认来源值及原因。C++ 在同一正式入库事务前重新执行评分纯函数，不只信任前端或 Python 结果。

`defect_observations.scale` 只能来自合同 1.2 的 `defect_scale`；`severity` 只表示提示级别，严禁写入病害标度。病害详细位置、标度和病害扣分分别写入 `defect_location`、`scale` 和 `defect_deduction`。
...
真实 Word 解析基线由环境门控测试
`tools-python/tests/importers/test_real_word_regression.py` 固化。设置
`BRIDGE_REPORT_REAL_WORD_PATH` 后断言病害、照片、图片文件、评分和构件编号数量；未设置时跳过，真实报告不进入 Git。
...
13. 同桥同年修订确认后旧版本失效、新版本生效。
14. 入库事务失败时不留下半截事实。
15. 前端能按 `需要处理` 和普通候选分组显示。
...
13. 能编辑并确认病害详细位置、标度和病害扣分，且 `severity` 不会写入病害标度。
14. 能展示构件评分来源值与规范复算值；不一致或无法复算时，未经显式选择和原因不能入库。
15. C++ 在正式事务前独立复算，并把构件评分及校验明细写入构件级 `condition_ratings`。
16. 真实 Word 原回归数量保持不变，且精确验证 `1-1#板=65`、`1-2#板=65`、`2-1#板=55.81`、上部承重构件 `86.62`、全桥 `85.61`。
...
- 2026-09-04：归并当前合同 5.0、关系化构件/评分树解析和系统自主评定边界；旧评分校对内容标为历史。
- 2026-08-28：合同升级为 5.0；构件解析、展开实例和评分树解析从草稿 JSON 移入关系表。
...
- 2026-07-12：辽宁国省干线真实 Word 验证基线为 25 条病害、31 个照片候选、36 个临时图片、31 个归档照片和 15 个评分项。
- 2026-07-13：契约升级设计为 1.2；加入详细位置、标度、扣分、构件评分复算与差异处理；修正 `severity` 不得映射为标度。
...
| 2026-07-14 | 合同 1.2 校对与确认实施完成：病害卡新增标度/扣分编辑并联动重算构件评分；评分区新增构件评分子表（来源分/复算分/差值/状态，"接受Word值/采用复算值"需必填原因，可撤销）；`ConfirmPlan` 停止 severity→scale 映射，改写 `defect_scale`/`defect_deduction`；预检新增 `component_score_pending_resolution`、`component_score_recalc_mismatch`、`component_score_deduction_incomplete`、`component_score_defect_reference_invalid`、`component_rating_duplicate_component` 并入库前独立复算；构件级 `condition_ratings`（迁移 003 三值校验列）随确认事务写入；待校对的 1.0/1.1 旧草稿标记 `legacy_pending_reparse` 只读、须重新解析（保存草稿校验存量版本，拒绝伪造 1.2 请求体绕过） | 实施合同 1.2 | `05-review-workspace`、`06-component-defect-archive` |

...
日期：2026-07-03
当前合同版本：5.0（2026-08-28 实施）

> 2026-09-04 现行说明：运行时只接受 `BridgeAnnualInspectionData 5.0`。5.0 在 4.0 已删除
> Word 评分和照片级状态的基础上，进一步把构件解析、解析目标、展开实例及评分树解析从
> `parsed_result_json` 移入 PostgreSQL 关系表。下文 1.x/4.0 的字段样例和评分章节仅作历史记录；
> 当前结构以 `contracts/bridge_annual_inspection_data.schema.json` 和本节为准。
...
- 顶层 `ratings`、Word 分数或 Word 扣分；
- `bridge_component_id` 和构件档案修订 ID；
- 评分树版本、评分树节点及其匹配证据；
- 构件或评分树匹配方式；
- 照片级 `match_status`、`review_status` 或逐张确认状态。

候选 JSON 只承载来源事实。C++ 在正式确认时组合 5.0 JSON 与关系表中的权威解析状态，生成正式
病害、尺寸和照片事实；正式评分随后由系统 evaluator 产生。Python、C++、TypeScript 和 Schema
都必须显式拒绝旧版本及已删除字段，不能静默兼容。
...
| 2026-08-10 | 运行时升级为 4.0 | 删除 Word 评分及照片级匹配、校对状态 | 03～06 |
| 2026-08-28 | 运行时升级为 5.0 | 构件解析和评分树解析移出整份候选 JSON，改由关系表维护 | 03～06、导入解析 |
| 2026-09-04 | 当前规格说明归并到 5.0 | 修正文档版本及候选/解析/正式事实边界 | 全部导入消费者 |

...
**决策点（已定）：**
1. **绑定位置**：导入记录上的独立步骤，绑定端点改写 `parsed_result_json`。
2. **绑定页形态**：**独立页面**（非导入详情内区块）。
   > **已被后续变更取代（2026-07-24，提交 `9f0594f`）**：独立页与年度检测页上的"构件绑定"链接均已删除，
   > 绑定改为校对工作台左侧导航的一个分区（位于"需要处理"与"病害与照片"之间），路由
   > `…/imports/:id/binding` 与 `componentBindingPath` 一并移除。理由：独立入口与校对页分区并存会形成重复路径。
3. **标记缺失表示**：`component_match_method="missing"` + `bridge_component_id=null`（不新增列/表）；**不记缺失原因文本**，只记状态。
4. **行分组键**：`(标准类别 id, 归一化编号)`。
5. **进入校对 vs 确认门槛**："进入校对"是前端软跳转（允许"稍后再绑"）；硬门槛在确认入库：未绑定且未标记缺失 → 阻塞（复用现有 blocker，`missing` 视为已处理）。
6. **§4-E 只读**：本计划一并做（Task 6）——校对页 `ComponentMatchField` 改只读展示 + "去绑定界面"入口。

...
- [x] 记录最大批次（163 组 / 489 条）的响应字节数：**身份部分实测 96.7 KB**，
      设计 §8.2 的 150–250 KB 估算已替换为实测值，测试另设 512 KB 上限防回归。
...
2026-08-26 在真实库（百股大桥）上逐条核对。

- [x] 打开工作台只发一个摘要请求，无逐观测候选请求风暴。
      —— 前端测试断言 `fetchThreadSuggestions` 零调用；实测首屏 1 个请求。
- [x] 144 个批次摘要按覆盖观测数降序，样例稳定不随刷新变化。
      —— 实测降序成立（150, 87, 57, 33, 22, 20 … 1, 1, 1）；两次请求的样例组指纹与
      `batch_id` 完全一致。
- [x] 所有卡片显示业务编号，可区分 `1#墩盖梁` 与 `10#墩盖梁`。
      —— 实测样例为 `1-21#铰缝` / `21-5#铰缝` / `9-5#铰缝`、`1-1#板` / `2-1#板` / `2-9#板`。
- [x] "铰缝·渗水泛碱·三年"批次一次事务建成 163 条线索、绑定 489 条观测。
      —— `total_ms=490`，`issues=0`；库内实为 163 条线索、489 条绑定，每条线索恰好 3 条观测。
- [ ] `bind` 批次逐组绑到各构件自己的线索，界面无批次级单一 BHXS 编号。
      —— **真实数据未覆盖**：冷启动时该桥 0 条线索，144 个批次全为 `create`。由前端测试
      （bind 批次卡无批次级 BHXS、展开后逐组显示各自编号）与后端测试覆盖，明年增量入库时
      才会在真实数据上出现。
- [x] 未绑定组与已有线索位置包含时进异常簇，不自动 `create`。
      —— 组对组一侧实测命中 3 例：`16#墩盖梁`（大小里程侧 ⊂ 大小里程侧及左悬臂底部）、
      `1#跨桥面铺装`（0#台顶 / 右侧行车道 同被 0#台顶，右侧行车道 包含，并查集连成一簇）、
      `19#跨桥面铺装`（右侧行车道 ⊂ 右侧行车道19#墩处）。组对已有线索一侧冷启动无线索可比，
      由后端测试覆盖。
- [x] 任一观测并发变化 → 整批失败，数据库无部分写入，`issues` 列出全部问题。
      —— 后端测试覆盖（`observation_revision_conflict`）。
- [x] 重试返回 `already_completed`，不重复建线索。
      —— 后端测试覆盖，日志实见 `status=already_completed threads_created=0`。
- [x] 空位置批量创建、命名无悬空分隔符、以后年度可精确匹配。
      —— 163 条线索命名均为 `渗水泛碱`，首尾及连续分隔符各 0 例；489 条已绑定观测的
      `(构件, 类型, coalesce(位置,''))` 与其线索键 489/489 全等，明年新观测会精确命中。
- [ ] 异常簇保留关系上下文，可完成合并 / 绑定 / 拆分。
      —— 界面与动作由前端测试覆盖（同屏展示全部位置组、同年多条并排、写法不一致须人确认、
      少勾即拆）。真实数据上那 10 组尚未处理，留给使用者按实际判断。
- [x] 剔除与"暂不处理"刷新后回到原批次（不持久化）。
      —— 前端测试覆盖；`apply` 后重新取摘要，异常簇仍为 10 组 / 23 条，未受批量影响。
- [x] 档案页为只读结果页，模块 06 不再有两套整理流程。
- [ ] 全部处理完后，档案页"尚未归入跨年线索"提示消失。
      —— **尚未全部处理**：本次只应用了最大的一个批次，仍余 708 条。档案页计数正确显示
      708，与工作台一致；提示归零需把余下 143 个批次与 6 个异常簇处理完。
- [x] 后端与前端测试相对 Task 0 基线只增不减；Task 11 的删除已逐条说明。
      —— 后端 692 → 769，前端 454 → 473。Task 11 删除的 5 条已逐条交代归属，其中两条守的
      保证本来落空，已补测试。

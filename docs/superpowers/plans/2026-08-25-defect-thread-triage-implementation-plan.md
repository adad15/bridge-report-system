...
- [ ] **改 `ThreadSuggestions.cpp`：`location_exact` 改用规范键比较，去掉
      "双方位置非空"的限制。** 这是设计 §6.4 明确要求同步调整的点——否则 166 个无位置
      铰缝在候选侧永远匹配不上，与批次侧的口径分裂。
...
- [ ] 补断言：最大批次是"铰缝 · 渗水泛碱 · 无位置 · 2024,2025,2026"，166 组 / 498 条。
...
- [ ] 记录最大批次（166 组 / 498 条）的响应字节数，与设计 §8.2 的 150–250 KB 估算比对，
      把实测值写回设计文档。
...
- [ ] "铰缝·渗水泛碱·三年"批次一次事务建成 166 条线索、绑定 498 条观测。

...
- `d0d725b`：无照片编号病害不再产生警告；
- `4819606`：病害序号、业务标签和待处理项精确定位；
- `2f4edd8`：009 导入删除审计、清理队列和数据库 smoke；
- `5614950`：删除模型、事务仓储、解析竞态、三类文件清理和管理员 API；
- `297826a`：年度资料卡片删除入口、影响弹窗、前端 API 和错误提示。

自动验证结果：

- Python 全量：118 项通过，1 项因未配置可选环境而跳过；
- 前端全量：260 项通过；
- C++ + PostgreSQL 全量：338 项通过；
- C++ Debug 构建通过；
- 前端 TypeScript 与生产构建通过；
- 数据库 002—009 幂等迁移及 smoke 通过，其中 009 导入记录删除 smoke 通过；
- 真实辽宁 Word 回归通过：25 条病害、31 个照片候选、36 个 Word 图片、31 张可归档照片、15 个评分项；
- `git diff --check` 通过，`.claude/`、`test-inputs/`、`test-output/`、`runtime/`、`backend-cpp/archive/`、`tools-python/archive/` 和本地截图均未纳入提交。

实现说明：删除原因、操作者快照、影响数量和文件清理状态保存在数据库审计中；业务记录永久删除且不可恢复。解析中记录删除后，迟到结果返回 `import_record_deleted`，不写回或重建记录。独占归档文件、临时 Word 和解析工作目录在事务提交后立即尝试清理，失败项由启动及定时任务继续重试；共享归档文件保留。

...
- [ ] 年度检测采用左侧年份、右侧工作台。
- [ ] 导入只在选定桥梁和年度后进行。
- [ ] 当前只有 Word 格式；界面使用中性“导入资料”命名。
- [ ] 多来源合并和冲突处理没有进入代码或数据库。
- [ ] 截图所示全屏校对工作台完整保留。
- [ ] 年度工作台不取得编辑锁。
- [ ] 校对退出返回原年度。
- [ ] 病害线索整理功能保留，但不占一级导航。
- [ ] Word 年度来自系统选择，不从文档自动改挂。
- [ ] 上传路径受控，响应不泄漏绝对路径或相对存储路径。
- [ ] 既有同桥同年唯一约束被复用，没有无必要迁移。
- [ ] 五个受保护本地目录未提交、未删除。
- [ ] Python、前端、C++/PostgreSQL、生产构建和真实 Word 回归全部通过。

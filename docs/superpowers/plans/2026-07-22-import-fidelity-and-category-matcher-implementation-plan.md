...
**决策点（需复核，尤其对照表）：**
1. **部件名称→类别对照表**：下表按 taxonomy 规范名给出基础映射 + 初拟同义写法。**同义写法最需你（领域）核对/补充**——报告实际怎么写"上部承重构件/主梁"、"伸缩缝/伸缩缝装置"等。
2. **ConfirmedAlias（人工确认别名）路径**：现有 `component_aliases` 表 + 别名自动匹配。spec §4-D 绑定"不做记忆持久化"。本计划**保留**别名自动匹配（编号 + 别名文本），退休与否留到绑定模块 #3 定。
3. **导入保真**：预计无需改存储（现管线已透传原文），本计划加**断言测试**锁定"不归一化存储"，若发现有裁剪再修。

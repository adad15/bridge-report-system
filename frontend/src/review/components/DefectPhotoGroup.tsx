...
export function DefectPhotoGroup({ draft, defect, sequenceNumber = 1, importRecordId, baseUrl, expanded, onToggle, dispatch, initialPhotoCandidateId, disabled = false, allowDelete = false, componentInventory = null }: DefectPhotoGroupProps) {
  ...
  return (
    <tbody id={reviewTargetId("defect", defect.candidate_id)} tabIndex={-1} className={groupClassName} aria-disabled={disabled} style={{ "--defect-category-color": categoryColor(defect.component_name) } as CSSProperties}>
      <tr className="defect-card-row">
        <td>
          ...
          <div className="defect-fact-grid">
            ...
            <input id={reviewTargetId("defect-field", defect.candidate_id, "photo_references")} aria-label="照片编号" disabled={disabled} value={defect.photo_references.map((reference) => reference.photo_number).join(", ")} onClick={keepRowOpen} onChange={(event) => dispatch({ type: "edit_defect_field", candidateId: defect.candidate_id, field: "photo_numbers", value: parsePhotoNumbers(event.target.value) })} />
            ...
            <input id={reviewTargetId("defect-field", defect.candidate_id, "measurement_text")} className="defect-fact-span" aria-label="尺寸原文" disabled={disabled} title={defect.measurement_text ?? ""} value={defect.measurement_text ?? ""} onClick={keepRowOpen} onChange={(event) => dispatch({ type: "edit_measurement_text", candidateId: defect.candidate_id, text: event.target.value })} />
            {expanded ? (
              <>

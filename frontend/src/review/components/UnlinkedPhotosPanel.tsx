...
export function UnlinkedPhotosPanel({ draft, importRecordId, baseUrl, selectedPhotoCandidateId, dispatch, disabled = false }: UnlinkedPhotosPanelProps) {
  ...
  return (
    <div className="unlinked-photos-panel">
      <h3>待关联及已处理照片</h3>
      ...
      <div className="defect-photo-review">
        ...
        <div className="defect-photo-thumbnails">{photos.map((photo) => <button key={photo.candidate_id} type="button" className={photo.candidate_id === active?.candidate_id ? "active" : ""} aria-label={`查看待处理照片 ${photo.photo_number}`} onClick={() => setActiveId(photo.candidate_id)}><img src={photoContentUrl(baseUrl, importRecordId, photo.candidate_id)} alt="" /><span>{photo.photo_number}</span></button>)}</div>
      </div>
    </div>
  );
}

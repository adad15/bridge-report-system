...
TEST(ReviewStatisticsTest, CountsDefectsAndPhotosOnly) {
    ...
    data["photos"].append(candidate("已确认"));
    data["photos"].append(candidate("已忽略"));
    ...
    EXPECT_EQ(stats.confirmed_count, 1);
    EXPECT_EQ(stats.modified_count, 1);
    EXPECT_EQ(stats.ignored_count, 1);
...
TEST(ReviewStatisticsTest, CountsOnlyNonEmptyDefectAndPhotoWarningArrays) {
    ...
    data["photos"].append(candidate("已修改", true));
...
TEST(ReviewStatisticsTest, ToJsonKeepsStableZeroRatingCountAndAllStatusCounts) {
    ...
    data["photos"].append(candidate("已忽略", true));
    ...
    EXPECT_EQ(json["ignored_count"].asInt(), 1);

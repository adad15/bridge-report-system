...
protected:
    void SetUp() override {
        ...
        profile_id_ = insert_id(
            ...
            "created_by_user_id,change_reason) values($1::uuid,$2::uuid,$3::uuid,'评定回执测试') returning id",
            technical_package_id_, maintenance_package_id, user_id_);
    ...
    std::string insert_formal_run(const std::string& year_id, const std::string& record_id,
        ...
        const auto run_id = insert_id(
            ...
            "$4::jsonb,$5,'{}'::jsonb,$6::uuid,$7,$8::uuid,$9::uuid,$10::uuid) returning id",
